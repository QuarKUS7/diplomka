...
# -*- coding: utf-8 -*-
"""
Created on Wed May  9 23:29:27 2018
...
def data():
    ...
    x_train,  x_test = dataset.iloc[0:train_size,:], dataset.iloc[train_size+valid_size:,:]
...
def create_model(x_train, x_test, dataset):
    ...
    model.fit_generator(generator(dataset, x_train, 64), steps_per_epoch=x_train.shape[0]/64, verbose=1, callbacks=None,#validation_data=generator(dataset, validX, 128), validation_steps=validX.shape[0], 
                        class_weight=None, max_queue_size=128, workers=1, use_multiprocessing=False, initial_epoch=0, epochs=1)
...
if __name__ == '__main__':
    ...
    loss, status, model=create_model(x_train, x_test, dataset)
